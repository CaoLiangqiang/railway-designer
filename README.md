...
- **线路管理**：创建多条线路，每条线路独立设置颜色和名称（名称和颜色均不可重复）
- **站点设计**：支持普通站和换乘站两种类型
- **轨道建设**：支持直线、一次折线、两次折线三种轨道形状
- **智能终点站识别**：自动识别线路端点作为终点站候选
- **终点站设置**：支持往返线和环线两种运行模式
- **列车模拟**：列车100%沿轨道运行，支持往返和环线模式
- **线路总览**：独立Tab展示所有线路站点，支持单条线路高亮显示
- **项目导出/导入**：保存和分享你的设计作品
...
下载 `轨道线路图设计器_v1.1.0_便携版.exe`，双击即可运行，无需安装。

### 方式二：安装版

下载 `轨道线路图设计器_v1.1.0_安装版.exe`，按照向导安装后使用。
...
### 核心算法
- **智能终点站识别**：基于站点连接数识别线路端点
- **列车路径计算**：100%沿轨道移动，支持往返和环线
...
### v1.1.0 (2026-02)
- **新增**：智能终点站识别功能，自动检测线路端点
- **新增**：线路总览Tab，展示所有线路站点列表
- **新增**：单条线路高亮显示功能
- **新增**：新建线路时名称和颜色唯一性验证
- **优化**：列车100%沿轨道移动
- **优化**：列车往返运行，到达终点站后返回始发站
- **修复**：模拟运行状态下禁止编辑操作
